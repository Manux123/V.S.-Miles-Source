...
class PlayState extends MusicBeatState
{
			...
			case 'miles_fondo':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/Background/bg miles-normal'));
						...
						var nubesTex = Paths.getSparrowAtlas('Miles/Background/bg miles nubes-normal');
						var nubes:FlxSprite = new FlxSprite(-600, -200);
						nubes.frames = nubesTex;
						nubes.animation.addByPrefix('idle', 'run run', 24, false);
						nubes.antialiasing = true;
						nubes.setGraphicSize(Std.int(nubes.width * 0.7));
						nubes.updateHitbox();
						add(nubes);

						if (PlayState.SONG.song.toLowerCase() == 'a-calm-fox' && PlayState.SONG.song.toLowerCase() == 'alerious' && PlayState.SONG.song.toLowerCase() == 'energy-max'){
			...
			case 'miles_fondo_tarde':
					{
						...
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/Background/tarde/bg miles atras-tarde'));
						bg.setGraphicSize(Std.int(bg.width * 0.7));
						bg.updateHitbox();
						bg.antialiasing = true;
						...
						var front:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/Background/tarde/bg miles adelante-tarde'));
						front.setGraphicSize(Std.int(front.width * 0.7));
						front.updateHitbox();
						front.antialiasing = true;
						front.scrollFactor.set(0.8, 0.8);
						front.active = false;
						add(front);

						var nubesTex = Paths.getSparrowAtlas('Miles/Background/tarde/bg miles nubes-tarde');
						var nubes:FlxSprite = new FlxSprite(-600, -200);
						nubes.frames = nubesTex;
						nubes.animation.addByPrefix('idle', 'run run', 24, false);
						nubes.antialiasing = true;
				...
				case 'miles_fondo_night':
					{
						defaultCamZoom = 0.6;
						curStage = 'miles_fondo_night';
	
						var sky:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/energy max/sky'));
						sky.setGraphicSize(Std.int(sky.width * 0.7));
						sky.updateHitbox();
						sky.antialiasing = true;
						sky.scrollFactor.set(0.9, 0.9);
						...
						fondo = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/energy max/bg miles-night'));
						fondo.setGraphicSize(Std.int(fondo.width * 0.7));
						fondo.updateHitbox();
						fondo.antialiasing = true;
						fondo.scrollFactor.set(0.8, 0.8);
						fondo.active = false;
						add(fondo);
	
						var nubesTex = Paths.getSparrowAtlas('Miles/energy max/bg miles nubes-night');
						var nubes:FlxSprite = new FlxSprite(-600, -200);
						nubes.frames = nubesTex;
						nubes.animation.addByPrefix('idle', 'run run', 24, false);
						nubes.antialiasing = true;
						...
						stagemiles = new FlxSprite(-600, -200).loadGraphic(Paths.image('Miles/energy max/stage'));
						stagemiles.setGraphicSize(Std.int(stagemiles.width * 0.7));
						stagemiles.updateHitbox();
						stagemiles.antialiasing = true;
						stagemiles.scrollFactor.set(0.8, 0.8);
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'miles_energymax':
						camFollow.y = dad.getMidpoint().y + 200;
						camFollow.x = dad.getMidpoint().x + 400;
					case 'miles_energymax_night':
						camFollow.y = dad.getMidpoint().y + 200;
						camFollow.x = dad.getMidpoint().x + 400;
				}
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat == 218 && SONG.song == 'A calm fox')
			{
				boyfriend.playAnim('hey', true);
			}

		if (curSong == 'energy max')
			{
				if (curBeat >= 47 && curBeat < 51 && camZooming)
				{
					FlxTween.tween(FlxG.camera, {zoom: -2.25}, 2);
		...
		switch (curStage)
		{
			case 'miles_fondo_night':
				...
				{	
					...
					var peopleTex = Paths.getSparrowAtlas('Miles/energy max/people');
					bgpeople = new FlxSprite(-600, -200);
					bgpeople.frames = peopleTex;
					bgpeople.animation.addByPrefix('idle', 'hey', 24, false);
					bgpeople.animation.play('idle', true);
					bgpeople.antialiasing = true;
					bgpeople.setGraphicSize(Std.int(bgpeople.width * 1));
